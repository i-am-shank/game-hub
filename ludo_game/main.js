var currPos=0, step=49.5, currcolor="", NumOfPaw="", num=0, clicked=false, currpawn="";
    ...
    for(var i=0; i<allcolor.length; i++) {
        for(var n=1; n<=4; n++) {
            ...
            if (firstPawn.style.top==secondPawn.style.top&&firstPawn.style.left==secondPawn.style.left&&currcolor!=allcolor[i]&&currPos+num<44) {
                count++, toKill=allcolor[i]+"pawn"+n;
                return toKill;
            }
        }
    }
...
function Stuck() {
    ...
    if(onboard[currpawn]==0 || currPos+num>44) {
        if(DontHaveOtherFree()||currPos+num>44) {
            ...
            badtext.innerText="You lose !!!", clicked=false;
...
function CheckForWinner() {
    if(pawnOut[currcolor] == 4) {
        ...
        dice.innerText="", dice.style.visibility="hidden";
        uselesstext1.innerText = "", uselesstext2.innerText = "";
...
function stepDown() {
    ...
    doc.style.top=(curr+step)+"px", currPos++;
}
function stepUp() {
...
function stepLeft() {
    ...
    doc.style.left=(curr-step)+"px", currPos++;
}
function stepRight() {
    var doc = document.getElementById(currpawn);
    var curr = Number(doc.style.left.replace(/[a-z]/g, ''));
...
var stepsRed=[], stepsYellow=[], stepsBlue=[], stepsGreen=[];
...
function ResetPawn(victim) {
    onboard[victim]=0, positions[victim]=0;
...
function randomNum() {
    if(!clicked) {
        ...
        var dice = document.getElementById("dice");
        dice.style.backgroundImage="url(Images/"+num+".jpg)", clicked = true;
    }
    if(num != 6&&DontHaveOtherFree()) {
        var bad = document.getElementById('badtext');
        bad.innerText="You lose !!!", window.setTimeout(changePlayer, 1000), clicked=false;
    }
}
function randomMove(Color, paw) {
    var text = document.getElementById('player');
    NumOfPaw=paw, currcolor=Color, currpawn=currcolor+"pawn"+NumOfPaw, currPos=positions[currpawn];
    if(num+currPos > 44) {
        Stuck();
    }
    else {
            ...
            if (text.innerText == currcolor) {
                if (onboard[currpawn] === 1 || num === 6) {
                    if (onboard[currpawn] === 0) {
                        ...
                        switch (Color) {
                            case "red":
                                doc.style.left=318+"px", doc.style.top=28+"px";
                                break;
                            case "yellow":
                                doc.style.left=219+"px", doc.style.top=523+"px";
                                break;
                            case "blue":
                                doc.style.left=516+"px", doc.style.top=325+"px";
                                break;
                            case "green":
                                doc.style.left=21+"px", doc.style.top=226+"px";
                                break;
                        }
                        onboard[currpawn]=1;
                    }
                        ...
                        switch(Color) {
                            case "red":
                                for(i=currPos; i<position+num; i++) { stepsRed[i](); } break;
                            case "yellow":
                                for(i=currPos; i<position+num; i++) { stepsYellow[i](); } break;
                            case "blue":
                                for (i=currPos; i<position+num; i++) { stepsBlue[i](); } break;
                            case "green":
                                for(i=currPos; i<position+num; i++) { stepsGreen[i](); } break;
                        }
                        positions[currpawn]=currPos;
                        var victim = HaveHover();
                        if(victim != false) { ResetPawn(victim); }
